"""
@Title:             topology_assistant_flexibleCoil.py
@Name:              Mando A Ramirez
@Date:              2023 06 20

@Description:       This script is an extension of the "topology_assistant.py" script that I use to make proteins
in the first iteration of the framework. This script adds to the framework by allowing the user to control how
"flexible" the coil segments are.

The purpose of this script is to find the right parameters to make coils flexible to see the desired biological
behavior : flexible coils in isolation, then fold upon multimerization
...
def pairs_helper(pStart, pStop, segment_type="linker", pair_type="1-4", P14_DAMP=1.0, P15_DAMP=1.0):
    """
    ...
    if pair_type == "1-4":
        ...
        if segment_type == "coil":
            for i1 in range(len(pairs_list)):
                pairs_string += f" {pairs_list[i1][0]}   {pairs_list[i1][1]}   1     {COIL_P14_C6*P14_DAMP:.10f}     {COIL_P14_C12*P14_DAMP:.10f}\n"
        else:
            for i1 in range(len(pairs_list)):
        ...
        if segment_type == "coil":
            for i1 in range(len(pairs_list)):
                pairs_string += f" {pairs_list[i1][0]}   {pairs_list[i1][1]}   1     {COIL_P15_C6*P15_DAMP:.10f}     {COIL_P15_C12*P15_DAMP:.10f}\n"
        else:
            for i1 in range(len(pairs_list)):
...
def angle_helper(anStart, anStop, segment_type="linker", A_DAMP=1.0):
    """

    :param anStart:
    :param anStop:
    :param segment_type:
    :param A_DAMP: [float] a value to dampen the original force constant value of the coil segment
    :return:
    """
    angle_string = ""
    angle_list = []
    ...
    if segment_type == "coil":
        for j in range(len(angle_list)):
            angle_string += f" {angle_list[j][0]}    {angle_list[j][1]}    {angle_list[j][2]}     1       {THETA_HELIX}        {THETA_HELIX_FORCE*A_DAMP}\n"
        return angle_string
    else:
        for j in range(len(angle_list)):
...
def dihedral_helper(dStart, dStop, multiplicity, segment_type="linker", T_DAMP=1.0):
    """
    ...
    if segment_type == "coil":
        if multiplicity == 1:
            for k in range(len(dihedral_list)):
                dihedral_string += f" {dihedral_list[k][0]}   {dihedral_list[k][1]}   {dihedral_list[k][2]}   {dihedral_list[k][3]}    1     {ALPHA1_HELIX}        {ALPHA1_HELIX_FORCE*T_DAMP}      {multiplicity}\n"
            return dihedral_string
        if multiplicity == 3:
            for k in range(len(dihedral_list)):
                dihedral_string += f" {dihedral_list[k][0]}   {dihedral_list[k][1]}   {dihedral_list[k][2]}   {dihedral_list[k][3]}    1     {ALPHA3_HELIX}        {ALPHA3_HELIX_FORCE*T_DAMP}      {multiplicity}\n"
            return dihedral_string
...
if __name__ == "__main__":
    ...
    parser.add_argument("-angle_damp", help="weaken the coil segment angle force constant by this factor",
                        type=float, default=1.0)
    parser.add_argument("-torsion_damp", help="weaken the coil segment torsion force constant by this factor",
                        type=float, default=1.0)
    parser.add_argument("-p14_damp", help="weaken the coil segment 1-4 pairs interactions by this factor",
                        type=float, default=1.0)
    parser.add_argument("-p15_damp", help="weaken the coil segment 1-5 pairs interactions by this factor",
                        type=float, default=1.0)
    ...
    if args.itp:
        write_itp(args.itp_filename, args.n, Segments, Positions,
                  args.angle_damp, args.torsion_damp, args.p14_damp, args.p15_damp)
