"""
@Title:             topology_assistant_nonspecific.py
...
@Updates:
2022 06 02 - changing atom specification so that all "A" beads get labeled as ILE, which will be useful when examining
the PDB file.

2022 06 10 - changed the exclusions to allow for self-molecule interactions. Exclusions are still included so that
A-beads within a coil segment cannot interact, but different coil segments within a single molecule are free to do so.
I am preserving the original exclusions code in case I want to come back to it.

2022 06 17 - renamed the file to reflect emphasis on INTERACTIONS, instead of exclusions.

2022 10 14 - renamed the file to reflect the current description of this model, which we're calling "nonspecific" interactions
"""
...
[ exclusions ]
; Exclude only A-A interactions within individual coil segments
; But different coil segments in the same molecule can interact.
; I.E. intra- && inter-chain coil interactions are permitted
"""
    coil_num = 1
...
def atom_helper(aStart, aStop, segment_type="linker", heptad_start=0):  # arguments provided as 1-indexed
    """
    ...
    if segment_type == "linker":    # control for making linker information
        ...
        for posc in range(aStart, aStop+1):        # uses 1-indexing
            if posc in a_posits:
                atom_string += f" {posc}   A    {posc}    ILE     CA    {posc}    0.000    {MASS}\n"
