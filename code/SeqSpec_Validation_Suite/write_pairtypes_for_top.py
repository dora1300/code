"""
@Title:             write_pairtypes_for_top.py
@Name:              Mando A Ramirez
@Date:              2025 12 01

@Description:       This script generates the pairtypes.itp file text, as well as saves that file,
for a sequence specific simulation. This is built from write_nonbond_params_for_top.py.
Like that other file, this can scale the epsilons and sigmas based on a scale factor.
"""
...
def write_pairtypes_itp(epsilon_file_name, sigma_table, scaling_factor_epsilon, scaling_factor_sigma):
    """
    Description:
    ...
    Returns:
        A string (very long string mind you) that contains all the nonbonded parameters for the topology.
    """

    # load the table of epsilons. This is mandatory! I can't make the .itp file
    # without it!
    epsilon_table = np.loadtxt(epsilon_file_name, 
                                delimiter=',',
                                encoding="utf-8-sig")    # might need to add encoding thing

    nonbond_params_text = f"""[ pairtypes ] 
;i    j     func    V(sigma)   W(epsilon)\n"""
    ...
    for I in range(len(aas_list)):
        for J in range(I, len(aas_list)):
            ...
            scaled_epsilon = epsilon_table[I][J] * scaling_factor_epsilon
            new_string = f"{aas_one_letter[I]}   {aas_one_letter[J]}   1       {scaled_sigma:.4f}    {scaled_epsilon:.4f}\n"
...
def load_calculated_sigmas(SIGMA_BY_MATRIX=True, SIGMA_MATRIX_FILE=None,
    ...
    if SIGMA_BY_MATRIX is True:
        ...
        sigma_matrix = np.loadtxt(SIGMA_MATRIX_FILE, 
                                  delimiter=",", 
                                  dtype=float,
                                  encoding="utf-8-sig")
        return sigma_matrix
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=f"A tool for writing nonbond_params.itp for protein sequence specific simulations.\n"
    "Please note that this script can be run standalone, or the tools can be imported in a separate script for use elsewhere.")
    ...
    itp_file_contents = write_pairtypes_itp(args.epsilon_file_name, array_of_sigmas, 
