"""
@Title:             importonly_itp_helper_functions.py
...
@Updates:
20250708            Updated the title for better precision about its use.
"""
import numpy as np
...
def generate_pairs_text(sequence_length, 
                        pair_type,
                        list_of_scalars,
                        pair_attractive, 
                        pair_repulsive,
    ...
    Arguments:
        list_of_scalars     :   This is the list of mapped backbone scalar values to adjust
                                a given pair's attractive and repulsive terms.
        combining_method    :   The method used to combined the backbone scalar values. Default
                                is the arithmetic mean. Other options will be added
    Returns:       
    """
...
"""
    for i in range(1, sequence_length-(increment-1)):
        # i is 1-indexed to match gromacs styling
        if combining_method == "mean":
            adjust = (list_of_scalars[i-1] + list_of_scalars[i+(increment-1)]) / 2.0
        else:
            adjust = (list_of_scalars[i-1] + list_of_scalars[i+(increment-1)]) / 2.0
        line_text = f" {i}    {i+increment}     1      {pair_attractive*adjust:.5e}       {pair_repulsive*adjust:.5e}\n"
        output_text += line_text
...
def generate_angles_text(sequence_length,
                         equil_value,
                         angle_force,
                         backbone_scalars,
                         combining_method="mean"):
    ...
    # The first thing I need to do is generate the list of indices (1-indexed!) that become the 
    # pseudoangles. I also must do something with the backbone scalar values for the residues
    # that comprise each pseudo angle. 
    # the container that has the indices for each pseudoangle also stores the combined backbone scalar
    # for those residues/indices
    pseudoangles = []
    for I in range(sequence_length-2):
        if combining_method == "mean":
            combined_scalar = (backbone_scalars[I] + backbone_scalars[I+1] + backbone_scalars[I+2]) / 3.0
        else:
            combined_scalar = (backbone_scalars[I] + backbone_scalars[I+1] + backbone_scalars[I+2]) / 3.0
        
        pseudoangles.append([I+1, I+2, I+3, combined_scalar])
...
def generate_torsions_text(sequence_length,
                           torsion_value,
                           torsion_force,
                           multiplicity,
                           backbone_scalars,
    ...
    """
    # The first thing I need to do is generate the list of indices (1-indexed!) that become the 
    # pseudotorsions. I also must do something with the backbone scalar values for the residues
    # that comprise each pseudo torsion. 
    # the container that has the indices for each pseudotorsion also stores the combined backbone scalar
    # for those residues/indices
    pseudotorsions = []
    for I in range(sequence_length-3):
        if combining_method == "mean":
            combined_scalar = (backbone_scalars[I] + backbone_scalars[I+1] + backbone_scalars[I+2] + backbone_scalars[I+3]) / 4.0
        else:
            combined_scalar = (backbone_scalars[I] + backbone_scalars[I+1] + backbone_scalars[I+2] + backbone_scalars[I+3]) / 4.0
        
        pseudotorsions.append([I+1, I+2, I+3, I+4, combined_scalar])


    output_text = f""";     multiplicity = {multiplicity}
"""
    # print(pseudotorsions)
