...
class Analysis(Sim_Setup):
    ...
    def Save_WIntClus_For_Mod(self, sysName, plot=False):
        """
        ...
        """
        try:
            dum_dat = self.Collect_WInt_CLUS_For_Mod(sysName, plotting=plot)
        ...
        for a_box in box_arr:
            ...
            if plotting:
                ...
                for a_run in range(run_num):
                    ...
                    plt.savefig(f"{plot_dir}{sysName}_{a_box}_noInt_{a_run+1}_PDF_separated.png", dpi=600)
                    plt.close(fig)
                colors = iter(cm.rainbow(np.linspace(0, 1, tot_temps)))
                for a_run in range(run_num):
                    ...
                    plt.savefig(f"{plot_dir}{sysName}_{a_box}_noInt_{a_run + 1}_PDF_stacked.png", dpi=600)
                    plt.close(fig)
        ...
        for a_box in box_arr:
            ...
            if plotting:
                ...
                for a_run in range(run_num):
                    ...
                    plt.savefig(f"{plot_dir}{sysName}_{a_box}_wInt_{a_run+1}_PDF_separated.png", dpi=600)
                    plt.close(fig)
                colors = iter(cm.rainbow(np.linspace(0, 1, tot_temps)))
                for a_run in range(run_num):
                    fig, ax = plt.subplots(figsize=(10,10))
                    ...
                    plt.savefig(f"{plot_dir}{sysName}_{a_box}_wInt_{a_run + 1}_PDF_stacked.png", dpi=600)
                    plt.close(fig)
            tot_raw_data.append(box_pdf_data)
        return tot_raw_data
    ...
    def Collect_WInt_CLUS_For_Mod(self, sysName, plotting=False):
        """
        ...
        for a_box in box_arr:
            ...
            for a_run in range(run_num):
                ...
                tot_raw_dat_temp.append(self.Read_CLUSFileFor(file_name))
            if plotting:
                plotting_data = np.average(tot_raw_dat_temp, 0)
                colors = iter(cm.rainbow(np.linspace(0, 1, tot_temps)))
                fig, axs = plt.subplots(tot_temps, figsize=(30, 30), sharex=True)
                for temp in range(tot_temps):
                    current_temp = key_dum['MC_TEMP'] + (key_dum['MC_DELTA_TEMP'] * temp)
                    co = next(colors)
                    temp_plotting_data = np.delete(plotting_data[temp], 0)
                    axs[temp].hist(temp_plotting_data, bins=mol_num,
                                   color=co, linewidth=5,
                                   label=f"Temp {current_temp:.2f}")
                    axs[temp].legend()
                    axs[temp].set_ylim(bottom=0, top=np.max(temp_plotting_data)+1)
                xlabels = []
                for n in np.arange(1, 1500+1, 100.):
                    xlabels.append(str(n))
                plt.xticks(np.arange(0, mol_num, 100.), labels=xlabels)
                plt.xlim(0, mol_num+1)
                plt.xlabel("Cluster size")
                plt.ylabel("Counts")
                plt.suptitle(f"WInt Box {a_box} cluster size histograms separated by temperature")
                plt.savefig(f"{plot_dir}{sysName}_{a_box}_wInt_{a_run+1}_ClusHist_separated.png", dpi=600)
                plt.close(fig)
            tot_raw_dat.append(tot_raw_dat_temp)
        return tot_raw_dat
...
#             zero_points = np.argwhere(not_run_ave == 0.).T[0]
            ...
            if plotting:
                ...
                plt.legend()
                plt.savefig(f"{plot_dir}{sysName}_{a_box}_pRDF_stacked.png", dpi=600)
                plt.close(fig)
